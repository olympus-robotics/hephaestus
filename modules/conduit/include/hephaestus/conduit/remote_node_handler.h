...
private:
  ...
  auto handleInputClient(heph::net::Socket client) -> exec::task<void>;
  auto handleOutputClient(heph::net::Socket client) -> exec::task<void>;
  ...
  auto createPublisherNode(Engine& engine, Node& node, heph::net::Socket socket, std::string name) {
    auto publisher = engine.template createNode<RemotePublisherNode<T>>(std::move(socket), std::move(name));
    publisher->input.connectTo(node);
  }

  template <typename T, typename Engine, typename InputT>
  auto createSubscriberNode(Engine& engine, InputT* input, heph::net::Socket socket) {
    auto subscriber = engine.template createNode<RemoteInputSubscriber<T>>(std::move(socket), input->name());
...
private:
  struct InputRegistryEntry {
    ...
    heph::UniqueFunction<void(heph::net::Socket)> subscriber_factory;
  };
  ...
  if constexpr (detail::ISOPTIONAL<ResultT>) {
    ...
    registered_outputs_.emplace(
        ...
        OutputRegistryEntry{ .type_info = heph::serdes::getSerializedTypeInfo<ValueT>().toJson(),
                             .publisher_factory =
                                 [this, &engine, &output](heph::net::Socket socket) {
    ...
    if constexpr (!std::is_same_v<void, ResultT>) {
      registered_outputs_.emplace(
          output.name(), OutputRegistryEntry{
                             .type_info = heph::serdes::getSerializedTypeInfo<ResultT>().toJson(),
                             .publisher_factory = [this, &engine, &output](heph::net::Socket socket) {
  ...
  if constexpr (std::tuple_size_v<ExecuteResultTupleT> == 1) {
    ...
    if constexpr (detail::ISOPTIONAL<ResultT>) {
      ...
      registered_outputs_.emplace(
          ...
          OutputRegistryEntry{ .type_info = heph::serdes::getSerializedTypeInfo<ValueT>().toJson(),
                               .publisher_factory =
                                   [this, &engine, &node](heph::net::Socket socket) {
      ...
      if constexpr (!std::is_same_v<void, ResultT>) {
        registered_outputs_.emplace(
            ...
            OutputRegistryEntry{ .type_info = heph::serdes::getSerializedTypeInfo<ResultT>().toJson(),
                                 .publisher_factory = [this, &engine, &node](heph::net::Socket socket) {
                                   createPublisherNode<ResultT>(engine, node, std::move(socket),
                                                                node.nodeName());
                                 } });
      }
    }
...
void RemoteNodeHandler::registerInput(Engine& engine, InputT* input) {
  ...
  registered_inputs_.emplace(
      input->name(),
      InputRegistryEntry{ .type_info = heph::serdes::getSerializedTypeInfo<ValueT>().toJson(),
                          .subscriber_factory = [this, &engine, input](heph::net::Socket socket) {
                            createSubscriberNode<ValueT>(engine, input, std::move(socket));
                          } });
}
