...
namespace heph::net {
template <bool SendAll> struct RecvT {
...
namespace internal {
template <bool RecvAll, typename Receiver> struct RecvOperation {
  ...
  void prepare(::io_uring_sqe *sqe) const {
    auto recv_size =
        std::min(socket->maximumRecvSize(), buffer.size() - transferred);
    auto to_transfer = buffer.subspan(transferred, recv_size);
    ::io_uring_prep_recv(sqe, socket->nativeHandle(), to_transfer.data(),
                         to_transfer.size(), 0);
  }
...
struct RecvSender : heph::concurrency::DefaultSenderExpressionImpl {
  static constexpr auto GET_COMPLETION_SIGNATURES =
      []<typename Sender>(Sender &&, heph::concurrency::Ignore = {}) noexcept {
        return stdexec::completion_signatures<
            stdexec::set_value_t(std::span<std::byte>),
  ...
  static constexpr auto GET_STATE = []<typename Sender, typename Receiver>(
    ...
    return OperationStateT{
        ring,
        RecvOperationT{socket, buffer, std::forward<Receiver>(receiver), 0}};
  };
...
template <bool RecvAll>
struct SenderExpressionImpl<heph::net::RecvT<RecvAll>>
    : heph::net::internal::RecvSender<RecvAll> {};
} // namespace heph::concurrency
