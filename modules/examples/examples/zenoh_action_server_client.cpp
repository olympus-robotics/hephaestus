...
auto main(int argc, const char* argv[]) -> int {
  ...
  try {
    ...
    auto status_update_cb = [](const heph::examples::types::Accumulator& sample) {
      LOG(INFO) << fmt::format("Received update: {}", sample);
    };

    static constexpr std::size_t START = 42;
    static constexpr std::size_t ITERATIONS = 10;
    const auto target = heph::examples::types::Accumulator{
      .value = START,
      .counter = ITERATIONS,
    };
    auto result_future = heph::ipc::zenoh::callActionServer<heph::examples::types::Accumulator,
