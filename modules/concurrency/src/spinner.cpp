...
Spinner::Spinner(SpinnerStateMachineCallback&& state_machine_callback,
                 std::optional<double> rate_hz /*= std::nullopt*/)
  : state_machine_callback_(std::move(state_machine_callback))
...
void Spinner::spin() {
  ...
  while (!stop_requested_.load()) {
    ...
    // Logic to handle periodic spinning at a fixed rate. Initialization, execution etc. are handled at the
    // same rate.
