...
TEST(WhenAllRange, Concurrent) {
  ...
  static constexpr std::size_t NUMBER_OF_SENDERS{ 1000 };
  for (std::size_t i = 0; i != NUMBER_OF_SENDERS; ++i) {
...
TEST(WhenAllRange, ConcurrentStop) {
  std::vector<AnySender<void>> senders;
  exec::static_thread_pool pool{ 4 };
  ...
  static constexpr std::size_t NUMBER_OF_SENDERS{ 1000 };
  for (std::size_t i = 0; i != NUMBER_OF_SENDERS; ++i) {
    // Introduce two "stopper"
    if (i == NUMBER_OF_SENDERS / 2 || i == NUMBER_OF_SENDERS / 4) {
      senders.emplace_back(stdexec::schedule(pool.get_scheduler()) |
...
TEST(WhenAllRange, ConcurrentError) {
  std::vector<AnySender<void>> senders;
  exec::static_thread_pool pool{ 4 };
  ...
  for (std::size_t i = 0; i != NUMBER_OF_SENDERS; ++i) {
    // Introduce two "stopper"
    if (i == NUMBER_OF_SENDERS / 2 || i == NUMBER_OF_SENDERS / 4) {
      senders.emplace_back(stdexec::schedule(pool.get_scheduler()) | stdexec::let_value([]() {
                             return stdexec::just_error(std::make_exception_ptr(std::runtime_error("meh")));
                           }));
    } else {
      senders.emplace_back(stdexec::schedule(pool.get_scheduler()) |
                           stdexec::then([&completed]() { ++completed; }));
