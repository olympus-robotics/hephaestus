...
private:
  void onPublisher(const ActorInfo& info);
  void onPublisherAdded(const ActorInfo& info);
  void onPublisherDropped(const ActorInfo& info);

private:
  SessionPtr session_;
  TopicFilter topic_filter_;

  std::unique_ptr<ActorDiscovery> discover_publishers_;
