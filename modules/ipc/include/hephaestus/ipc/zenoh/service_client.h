...
ServiceClient<RequestT, ReplyT>::ServiceClient(SessionPtr session, TopicConfig topic_config,
  ...
  throwExceptionIf<FailedZenohOperation>(
      result != 0,
      fmt::format("[ServiceClient '{}'] failed to create zenoh querier, err {}", topic_config_.name, result));
  ...
  throwExceptionIf<FailedZenohOperation>(
      result != 0, fmt::format("[ServiceClient {}] failed to create liveliness token, result {}",
...
auto ServiceClient<RequestT, ReplyT>::call(const RequestT& request) -> std::vector<ServiceResponse<ReplyT>> {
  ...
  throwExceptionIf<FailedZenohOperation>(result != 0,
                                         fmt::format("Failed to call service on '{}'", topic_config_.name));
