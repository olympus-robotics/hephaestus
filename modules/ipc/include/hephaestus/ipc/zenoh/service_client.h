...
ServiceClient<RequestT, ReplyT>::ServiceClient(SessionPtr session, TopicConfig topic_config,
  ...
  HEPH_PANIC_IF(result != 0, "[ServiceClient '{}'] failed to create zenoh querier, err {}",
  ...
  HEPH_PANIC_IF(result != 0, "[ServiceClient {}] failed to create liveliness token, result {}",
                topic_config_.name, result);
}
...
auto ServiceClient<RequestT, ReplyT>::call(const RequestT& request) -> std::vector<ServiceResponse<ReplyT>> {
  ...
  HEPH_PANIC_IF(result != 0, "Failed to call service on '{}'", topic_config_.name);
