...
template <class T>
class RandomTypeTests : public heph::test_utils::HephTest {};
...
TYPED_TEST(RandomTypeTests, DeterminismTest) {
  auto [mt_a, mt_b] = createPairOfIdenticalRNGs();
  EXPECT_EQ(random::random<TypeParam>(mt_a), random::random<TypeParam>(mt_b));
}
