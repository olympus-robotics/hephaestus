...
void WebsocketBridge::start() {
  ...
  log(INFO, "[WS Bridge] - Starting ...");

  {
    log(INFO, "[WS Server] - Starting ...");
...
void WebsocketBridge::stop() {
...

  {
    ...
    ws_server_->stop();
    log(INFO, "[WS Server] - OFFLINE ...");
  }

  log(INFO, "[WS Bridge] - OFFLINE");
}
...
void WebsocketBridge::callback_IpcGraph_TopicFound(const std::string& topic,
  ...
  CHECK(ipc_graph_);
  log(heph::DEBUG, "[WS Bridge] - New topic will be added  ...", "topic", topic, "type_name", type_info.name);

  if (state_.hasIpcTopicMapping(topic)) {
    if (config_.ws_server_verbose_bridge_state) {
      state_.printBridgeState();
    }
    log(WARN, "[WS Bridge] - Topic is already advertised! There are likely multiple publishers!", "topic",
        topic);
    return;
  }
  ...
  log(INFO, "[WS Bridge] - New topic added successfully.", "topic", topic, "type_name", type_info.name,
      "channel_id", new_channel_id);
}

void WebsocketBridge::callback_IpcGraph_TopicDropped(const std::string& topic) {
  log(heph::DEBUG, "[WS Bridge] - Topic will be dropped  ...", "topic", topic);
  if (!state_.hasIpcTopicMapping(topic)) {
    if (config_.ws_server_verbose_bridge_state) {
      state_.printBridgeState();
    }
    log(WARN, "[WS Bridge] - Topic is already dropped! There are likely multiple publishers!", "topic",
        topic);
    return;
  }
  ...
  log(INFO, "[WS Bridge] - Topic dropped successfully.", "topic", topic);
}
...
void WebsocketBridge::callback_IpcGraph_ServiceFound(const std::string& service_name,
  ...
  log(heph::DEBUG, "[WS Bridge] - Service will be added  ...", "service_name", service_name,
  ...
  log(INFO, "[WS Bridge] - Service was added successfully.", "service_name", service_name, "new_service_id",
      new_service_id);
}

void WebsocketBridge::callback_IpcGraph_ServiceDropped(const std::string& service_name) {
  log(heph::DEBUG, "[WS Bridge] - Service will be dropped  ...", "service_name", service_name);

  if (!state_.hasIpcServiceMapping(service_name)) {
    if (config_.ws_server_verbose_bridge_state) {
      state_.printBridgeState();
    }
    log(WARN, "[WS Bridge] - Service is already dropped! There are likely multiple service servers!",
  ...
  log(INFO, "[WS Bridge] - Service dropped successfully.", "service_name", service_name);
}
...
void WebsocketBridge::callback_Ipc_ServiceResponsesReceived(
  ...
  if (!state_.hasWsServiceMapping(service_id)) {
    log(ERROR, "[WS Bridge] - Received service response for unknown or dropped service id!", "call_id",
        call_id, "service_id", service_id);
    return;
  }
  ...
  if (client_handle_w_name_opt.has_value()) {
    ...
    if (!client_handle_w_name_lookup.has_value()) {
      log(ERROR, "[WS Bridge] - No client handle found for service call id. Dropping service response.",
  ...
  if (responses.empty()) {
    ...
    log(ERROR, msg, "service_name", service_name, "service_id", service_id, "call_id", call_id);
    ws_server_->sendServiceFailure(client_handle, service_id, call_id, msg);
    return;
  }

  if (responses.size() > 1) {
    log(WARN, "[WS Bridge] - Multiple service responses received. Forwarding first only.", "response_count",
        responses.size(), "service_name", service_name, "service_id", service_id);
  }
  ...
  if (response.topic != service_name) {
    ...
    log(ERROR, msg, "service_id", service_id, "call_id", call_id, "response_topic", response.topic,
        "service_name", service_name, "service_name_in_response", response.topic);
    ws_server_->sendServiceFailure(client_handle, service_id, call_id, msg);
    return;
  }
  ...
  log(INFO, "[WS Bridge] - Client received service response successfully.", "client_name", client_name,
      "service_name", service_name, "service_id", service_id, "call_id", call_id, "sync_or_async",
      sync_service_call);
}
...
void WebsocketBridge::callback_Ws_Log(WsLogLevel level, const char* msg) {
  switch (level) {
    case WsLogLevel::Debug:
      log(DEBUG, fmt::format("[WS Server] - {}", msg));
      break;
    case WsLogLevel::Info:
      log(INFO, fmt::format("[WS Server] - {}", msg));
      break;
...
void WebsocketBridge::callback_Ws_Subscribe(WsChannelId channel_id, const WsClientHandle& client_handle) {
  ...
  log(INFO, "[WS Bridge] - Client subscribes to topic...", "client_name", client_name, "topic", topic,
      "channel_id", channel_id);

  state_.addWsChannelToClientMapping(channel_id, client_handle, client_name);

  if (ipc_entity_manager_->hasSubscriber(topic)) {
    log(INFO, "[WS Bridge] - Client subscribed to topic successfully [IPC SUB EXISTS].", "client_name",
  ...
  if (!topic_type_info.has_value()) {
    log(ERROR, "[WS Bridge] - Could not subscribe because failed to retrieve type", "topic", topic,
        "channel_id", channel_id);
    return;
  }
  ...
  log(INFO, "[WS Bridge] - Client subscribed to topic successfully. [IPC SUB ADDED]", "client_name",
      client_name, "topic", topic, "channel_id", channel_id);
  if (config_.ws_server_verbose_bridge_state) {
    state_.printBridgeState();
  }
...
void WebsocketBridge::callback_Ws_Unsubscribe(WsChannelId channel_id, const WsClientHandle& client_handle) {
  ...
  log(INFO, "[WS Bridge] - Client unsubscribes from topic ...", "client_name", client_name, "topic", topic,
      "channel_id", channel_id);
  ...
  if (!state_.hasWsChannelWithClients(channel_id)) {
    if (ipc_entity_manager_->hasSubscriber(topic)) {
      ipc_entity_manager_->removeSubscriber(topic);
      log(INFO, "[WS Bridge] - Client unsubscribed from topic successfully. [IPC SUB REMOVED]", "client_name",
          client_name, "topic", topic, "channel_id", channel_id);
    } else {
      log(INFO, "[WS Bridge] - Client unsubscribed from topic successfully. [IPC SUB NOT FOUND]",
          "client_name", client_name, "topic", topic, "channel_id", channel_id);
    }
  } else {
    log(INFO, "[WS Bridge] - Client unsubscribed from topic successfully. [IPC SUB STILL NEEDED]",
        "client_name", client_name, "topic", topic, "channel_id", channel_id);
  }
  if (config_.ws_server_verbose_bridge_state) {
    state_.printBridgeState();
...
void WebsocketBridge::callback_Ws_ClientAdvertise(const WsClientChannelAd& advertisement,
  ...
  log(INFO, "[WS Bridge] - Client advertises topic ...", "client_name", client_name, "topic", topic,
      "channel_id", channel_id);

  if (state_.hasClientChannelMapping(channel_id)) {
    log(ERROR, "[WS Bridge] - Client tried to advertise topic but the channel already exists!", "client_name",
        client_name, "channel_id", channel_id, "topic", topic);
    return;
  }
  ...
  if (state_.hasTopicToClientChannelMapping(topic)) {
    ...
    log(WARN, "[WS Bridge] - Multiple clients advertise the same topic!", "client_name", client_name,
        "channel_id", channel_id, "topic", topic, "num_clients", (other_channels.size() + 1),
        "other_channel_ids", other_channels_str);
  }
  ...
  if (!type_info.has_value()) {
    log(ERROR, "[WS Bridge] - Failed to convert client advertisement to valid IPC type info!", "topic", topic,
        "channel_id", channel_id);
    return;
  }
  ...
  if (!ipc_entity_manager_->hasPublisher(topic)) {
    ...
    log(INFO, "[WS Bridge] - Client advertised topic successfully [IPC PUB ADDED]", "client_name",
        client_name, "topic", topic, "channel_id", channel_id);
  } else {
    log(INFO, "[WS Bridge] - Client advertised topic successfully [IPC PUB EXISTS]", "client_name",
        client_name, "topic", topic, "channel_id", channel_id);
  }
...
void WebsocketBridge::callback_Ws_ClientUnadvertise(WsClientChannelId client_channel_id,
  ...
  log(INFO, "[WS Bridge] - Client unadvertises topic ...", "client_name", client_name, "topic", topic,
      "client_channel_id", client_channel_id);

  if (topic.empty()) {
    log(ERROR, "[WS Bridge] - Client tried to unadvertise channel but the channel is unknown!", "client_name",
        client_name, "channel_id", client_channel_id);
    return;
  }
  ...
  if (!client_handle_with_name.has_value()) {
    log(ERROR, "[WS Bridge] - Client tried to unadvertise topic but the channel is not owned by this client!",
  ...
  if (!state_.hasClientChannelsForTopic(topic)) {
    if (ipc_entity_manager_->hasPublisher(topic)) {
      ...
      log(INFO, "[WS Bridge] - Client dropped topic successfully. [IPC PUB REMOVED]", "client_name",
          client_name, "topic", topic, "client_channel_id", client_channel_id);
    } else {
      log(INFO, "[WS Bridge] - Client dropped topic successfully. [IPC PUB NOT FOUND]", "client_name",
          client_name, "topic", topic, "client_channel_id", client_channel_id);
    }
  } else {
    log(INFO, "[WS Bridge] - Client dropped topic successfully. [IPC PUB STILL NEEDED]", "client_name",
        client_name, "topic", topic, "client_channel_id", client_channel_id);
  }
...
void WebsocketBridge::callback_Ws_ClientMessage(const WsClientMessage& message,
  ...
  if (!ipc_entity_manager_->hasPublisher(topic)) {
    log(ERROR, "[WS Bridge] - Client sent message for dropped topic!", "client_name", client_name, "topic",
        topic);
    return;
  }

  if (message.data.empty()) {
    log(ERROR, "[WS Bridge] - Client sent empty message!", "client_name", client_name, "topic", topic,
        "channel_id", channel_id);
    return;
  }
  ...
  if (message.data.size() < NUM_MESSAGE_HEADER_BYTES) {
    log(ERROR, "[WS Bridge] - Client sent message with insufficient data!", "client_name", client_name,
        "topic", topic, "channel_id", channel_id, "message_size", message.data.size());
    return;
  }
  ...
  if (opcode != static_cast<uint8_t>(WsClientBinaryOpCode::MESSAGE_DATA)) {
    log(ERROR, "[WS Bridge] - Client sent message with unexpected opcode!", "client_name", client_name,
        "topic", topic, "channel_id", channel_id, "opcode", static_cast<int>(opcode));
    return;
  }
  if (parsed_channel_id != channel_id) {
    log(ERROR, "[WS Bridge] - Client sent message with unexpected channel id!", "client_name", client_name,
        "topic", topic, "channel_id", channel_id, "parsed_channel_id", parsed_channel_id);
    return;
  }
  ...
  if (!ipc_entity_manager_->publishMessage(topic, message_data)) {
    log(ERROR, "[WS Bridge] - Failed to publish client message!", "client_name", client_name, "topic", topic,
        "channel_id", channel_id);
  }
}
...
void WebsocketBridge::callback_Ws_ServiceRequest(const WsServiceRequest& request,
  ...
  if (!state_.hasWsServiceMapping(request.serviceId)) {
    ...
    log(ERROR, msg, "service_id", request.serviceId, "call_id", request.callId, "client_name", client_name);
  ...
  log(INFO, "[WS Bridge] - Client sent service request ...", "client_name", client_name, "service_name",
      service_name, "service_id", service_id, "call_id", call_id);
  ...
  if (config_.ipc_service_service_request_async) {
    ...
    auto response_callback = [this, service_id, call_id](const RawServiceResponses& responses) -> void {
      log(INFO, "[WS Bridge] - Service response callback triggered for service [ASYNC]", "num_responses",
          responses.size(), "service_id", service_id, "call_id", call_id);
    ...
    log(INFO, "[WS Bridge] - Client service request dispatched [ASYNC]", "client_name", client_name,
